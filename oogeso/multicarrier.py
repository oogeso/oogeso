...
#        return df
    ...
    def saveOptimisationResult(self,timestep,store_duals=None):
        ...
        co2 = [pyo.value(milp_compute.compute_CO2(self.instance,timesteps=[t]))
                for t in range(timelimit)]
        ...
        for t in range(timelimit):
            for d in devs_elout:
                rescap = pyo.value(milp_compute.compute_elReserve(
        ...
        if node in model.paramNodeDevices:
            ...
            for dev_model in devmodels:
                carriers_used = [item for sublist in
                     list(all_devmodels[dev_model].values())
                     for item in sublist]
        ...
        for k,edge in model.paramEdge.items():
            ...
            if carrier in ['gas','oil','wellstream','water']:
                ...
                if var in ["inner","outer"]:
                    p_out_comp = milp_compute.compute_edge_pressuredrop(model,edge=k,
                    ...
                    p_out_comp2 = milp_compute.compute_edge_pressuredrop(model,edge=k,
                             p1=var_p_in,Q=Q,linear=False)
                    p_out_comp2_linear = pyo.value(
                        milp_compute.compute_edge_pressuredrop(model,edge=k,
