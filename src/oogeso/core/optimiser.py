...
#                logging.debug("Excluding device {}".format(dev_id))
        ...
        edgelist = {edge_id:edge.params
            for edge_id,edge in self.all_edges.items()
            if edge.params['type']=='el'}
        coeffB,coeffDA = el_calc.computePowerFlowMatrices(
            nodelist,edgelist,baseZ=1)
        ...
        model.paramProfiles = pyo.Param(model.setProfile,model.setHorizon,
            within=pyo.Reals, mutable=True,initialize=1)
        model.paramDeviceIsOnInitially = pyo.Param(model.setDevice,
            mutable=True,within=pyo.Binary,initialize=1)
    ...
    def _specifyConstraints(self):
        ...
        if (('emissionRateMax' in params_generic) and
            ...
            model.constrO_emissionrate = pyo.Constraint(
                model.setHorizon,
                rule=self._rule_emissionRateLimit)
        else:
        ...
        if (('emissionIntensityMax' in params_generic) and
            ...
            model.constrO_emissionintensity = pyo.Constraint(
                model.setHorizon,
                rule=self._rule_emissionIntensityLimit)
        else:
        ...
        if (('elBackupMargin' in params_generic) and
            ...
            model.constrO_elBackupMargin = pyo.Constraint(
                model.setDevice,model.setHorizon,
    ...
    def updateOptimisationModel(self,timestep,profiles,first=False):
        ...
        for prof in self.pyomo_instance.setProfile:
            for t in range(timesteps_use_actual): #0,1,2,3
                self.pyomo_instance.paramProfiles[prof,t] = (
                        profiles['actual'].loc[timestep+t,prof])
            for t in range(timesteps_use_actual,horizon):
                self.pyomo_instance.paramProfiles[prof,t] = (
                        profiles['forecast'].loc[timestep+t,prof])
...
#            return sumStorPmax
    ...
    def compute_CO2_intensity(self,model,timesteps=None):
        ...
        co2_kg_per_time = self.compute_CO2(
                model,devices=None,timesteps=timesteps)
        flow_oilequivalents_m3_per_time = self.compute_oilgas_export(
                model,timesteps)
    ...
    def compute_exportRevenue(self,model,carriers=None,timesteps=None):
        ...
        return self.compute_export(model,value="revenue",carriers=carriers,
            timesteps=timesteps)

    def compute_oilgas_export(self,model,timesteps=None):
        '''Export volume (Sm3oe/s)'''
        return self.compute_export(model,value="volume",carriers=['oil','gas'],
            timesteps=timesteps)
    ...
    def getDevicesInout(self,carrier_in=None,carrier_out=None):
        ...
        for d,dev_obj in self.all_devices.items():
            ok_in = ((carrier_in is None)
                        or (carrier_in in dev_obj.carrier_in))
            ok_out = ((carrier_out is None)
                        or (carrier_out in dev_obj.carrier_out))
            if ok_in and ok_out:
                devs.append(d)
        return devs
