...
_devmodels = {
    ...
    "separator": {"in": ["wellstream", "el", "heat"], "out": ["oil", "gas", "water"]},
    "separator2": {
        "in": ["oil", "gas", "water", "el", "heat"],
        "out": ["oil", "gas", "water"],
        "serial": ["oil", "gas", "water"],
    },
    "well_production": {"in": [], "out": ["wellstream"]},
    "well_gaslift": {"in": ["gas"], "out": ["oil", "gas", "water"], "serial": ["gas"]},
    "sink_gas": {"in": ["gas"], "out": []},
    "sink_oil": {"in": ["oil"], "out": []},
    "sink_el": {"in": ["el"], "out": []},
    ...
    "pump_oil": {"in": ["oil", "el"], "out": ["oil"], "serial": ["oil"]},
    "pump_water": {"in": ["water", "el"], "out": ["water"], "serial": ["water"]},
    "pump_wellstream": {
        "in": ["wellstream", "el"],
        "out": ["wellstream"],
        "serial": ["wellstream"],
...
def getDevicePower(model, dev, t):
    ...
    if devmodel in ["gasturbine", "source_el", "fuelcell"]:
        ...
        devpower = -(
            model.varDeviceFlow[dev, "el", "in", t] * eta
...
def compute_CO2(model, devices=None, timesteps=None):
    ...
    for d in devices:
        ...
        if devmodel in ["gasturbine", "gasheater"]:
            thisCO2 = (
                sum(model.varDeviceFlow[d, "gas", "in", t] for t in timesteps)
                * gasflow_co2
            )
            ...
            thisCO2 = (
                sum(
                    model.varDeviceFlow[d, "el", "out", t]
...
def compute_startup_costs(model, devices=None, timesteps=None):
    ...
    for d in devices:
        ...
        if "startupCost" in model.paramDevice[d]:
            ...
            thisCost = (
                sum(model.varDeviceStarting[d, t] for t in timesteps) * startupcost
            )
            start_stop_costs += thisCost
        if "shutdownCost" in model.paramDevice[d]:
            shutdowncost = pyo.value(model.paramDevice[d]["shutdownCost"])
            thisCost = (
                sum(model.varDeviceStopping[d, t] for t in timesteps) * shutdowncost
            )
            start_stop_costs += thisCost
    # get average per sec:
    deltaT = model.paramParameters["time_delta_minutes"] * 60
    ...
    for dev in model.setDevice:
        ...
        for c in carriers_incl:
            ...
            if price_param in model.paramDevice[dev]:
                inflow = sum(model.varDeviceFlow[dev, c, "in", t] for t in timesteps)
                if value == "revenue":
...
def compute_CO2_intensity(model, timesteps=None):
    ...
    co2_kg_per_time = compute_CO2(model, devices=None, timesteps=timesteps)
    flow_oilequivalents_m3_per_time = compute_oilgas_export(model, timesteps)
...
def compute_compressor_demand(
    model, dev, linear=False, Q=None, p1=None, p2=None, t=None
):
    # power demand depends on gas pressure ratio and flow
    # See LowEmission report DSP5_2020_04 for description
    if model.paramDevice[dev]["model"] not in ["compressor_el", "compressor_gas"]:
...
def compute_pump_demand(
    model, dev, linear=False, Q=None, p1=None, p2=None, t=None, carrier="water"
):
    devmodel = model.paramDevice[dev]["model"]
...
def compute_edge_pressuredrop(model, edge, p1, Q, method=None, linear=False):
    ...
    if ("pressure.{}.out".format(carrier) in model.paramNode[n_from]) & (
        ...
        p0_from = model.paramNode[n_from]["pressure.{}.out".format(carrier)]
        p0_to = model.paramNode[n_to]["pressure.{}.in".format(carrier)]
    ...
    if method is None:
        ...
        if ("viscosity" in model.paramCarriers[carrier]) & (not linear):
    ...
    for d in alldevs:
        ...
        if "el" in inout[devmodel]["out"]:
            ...
            if devmodel in ["gasturbine"]:
                ...
                # even if Pmax=2 MW)
                maxValue = (
                    model.varDeviceStoragePmax[d, t]
...
def OBSOLETE_compute_elBackup(model, t, exclude_device=None):
    ...
    for d in alldevs:
        devmodel = model.paramDevice[d]["model"]
        if ("el" in inout[devmodel]["out"]) or ("el" in inout[devmodel]["in"]):
            el_devices.append(d)
