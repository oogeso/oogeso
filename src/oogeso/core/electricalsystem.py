...
def computePowerFlowMatrices(df_node, df_edge, baseZ=1):
    ...
    G = nx.MultiDiGraph()
    edges = [
        (df_branch["nodeFrom"][i], df_branch["nodeTo"][i], i, {"i": i, "b": b[i]})
        for i in df_branch.index
    ]
    G.add_nodes_from(df_node.index)
    G.add_edges_from(edges)
    A_incidence_matrix = -nx.incidence_matrix(
        G, oriented=True, nodelist=df_node.index, edgelist=edges
    ).T
    # Diagonal matrix
    ...
    Bf = -nx.incidence_matrix(
        G, oriented=True, nodelist=df_node.index, edgelist=edges, weight="b"
    ).T
