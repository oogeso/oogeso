...
class Simulator():
    ...
    def runSimulation(self,solver,profiles,timerange=None,timelimit=None,store_duals=None,
        write_yaml=False):
    ...
    def _saveOptimisationResult(self,timestep,store_duals=None):
        ...
        varDeviceFlow = self._getVarValues(pyomo_instance.varDeviceFlow,
        ...
        varDeviceIsPrep = self._getVarValues(pyomo_instance.varDeviceIsPrep,
              names=('device','time'))
        varDeviceIsOn = self._getVarValues(pyomo_instance.varDeviceIsOn,
              names=('device','time'))
        varDeviceStorageEnergy = self._getVarValues(pyomo_instance.varDeviceStorageEnergy,
              names=('device','time'))
        varDeviceStoragePmax = self._getVarValues(pyomo_instance.varDeviceStoragePmax,
              names=('device','time'))
        varDeviceStarting = self._getVarValues(pyomo_instance.varDeviceStarting,
              names=('device','time'))
        varDeviceStopping = self._getVarValues(pyomo_instance.varDeviceStopping,
              names=('device','time'))
        varEdgeFlow = self._getVarValues(pyomo_instance.varEdgeFlow,
              names=('edge','time'))
        varElVoltageAngle = self._getVarValues(pyomo_instance.varElVoltageAngle,
              names=('node','time'))
        varPressure = self._getVarValues(pyomo_instance.varPressure,
              names=('node','carrier','terminal','time'))
        varTerminalFlow = self._getVarValues(pyomo_instance.varTerminalFlow,
        ...
        co2 = [pyo.value(self.optimiser.compute_CO2(pyomo_instance,timesteps=[t]))
                for t in range(timelimit)]
        ...
        for t in range(timelimit):
            for d in devs_elout:
                rescap = pyo.value(self.optimiser.compute_elReserve(
