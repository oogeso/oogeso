...
class NetworkNode:
    ...
    def defineConstraints(self):
        ...
        constrTerminalEnergyBalance = pyo.Constraint(
            model.setCarrier,['in','out'],
            model.setHorizon,
            rule=self._rule_terminalEnergyBalance)
        setattr(model,'constrN_{}_{}'.format(
            self.node_id,'energybalance'),constrTerminalEnergyBalance)

        constr_ElVoltageReference = pyo.Constraint(model.setHorizon,
            rule=self._rule_elVoltageReference)
        setattr(model,'constrN_{}_{}'.format(
            self.node_id,'voltageref'),constr_ElVoltageReference)

        constrPressureAtNode = pyo.Constraint(model.setCarrier,
            model.setHorizon, rule=self._rule_pressureAtNode)
        setattr(model,'constrN_{}_{}'.format(
            self.node_id,'nodepressure'),constrPressureAtNode)

        constrPressureBounds = pyo.Constraint(
            model.setTerminal,model.setCarrier,
            model.setHorizon,rule=self._rule_pressureBounds)
        setattr(model,'constrN_{}_{}'.format(
            self.node_id,'pressurebound'),constrPressureBounds)
