...
def OBSOLETEread_data_from_xlsx(filename, includeforecast=True):
    ...
    """
    dfs = {}
    dfs["node"] = _convert_xls_input(
        pd.read_excel(filename, sheet_name="node"),
        columns=["id", "name"],
        index_col="id",
    )
    dfs["edge"] = _convert_xls_input(
        pd.read_excel(filename, sheet_name="edge"),
        columns=["id", "include", "nodeFrom", "nodeTo", "type", "length_km"],
        index_col="id",
    )
    dfs["device"] = _convert_xls_input(
        pd.read_excel(filename, sheet_name="device"),
        columns=["id", "include", "node", "model", "name"],
        index_col="id",
    )
    ...
    dfs["carriers"] = _convert_xls_input(
        pd.read_excel(filename, sheet_name="carriers"), columns=["id"], index_col="id"
    )
...
def OBSOLETE_oogeso_dict_to_df(data_dict):
    ...
    def d2d(key):
        ...
        without parameters"""
        return pd.DataFrame.from_dict(data_dict[key], orient="index").reindex(
            data_dict[key].keys()
        )
...
def OBSOLETEcreate_initdata(data_dict=None, dfs=None):
    ...
    dfs["device"]["name"].fillna(
        pd.Series(data=dfs["device"].index, index=dfs["device"].index), inplace=True
    )
    ...
    for i, edg in df_edge.iterrows():
        ...
        try:
            ...
            if (not np.isnan(existing_p_to)) and (existing_p_to != p_to):
                msg = (
                    "Input data edge pressure to values are" " inconsistent (edge={})"
                ).format(i)
                is_consistent = False
        except:
            pass
    ...
    data["paramDeviceIsOnInitially"] = {
        k: v["isOn_init"] for k, v in data["paramDevice"].items() if "isOn_init" in v
    }
    data["paramDevicePowerInitially"] = {
        k: v["P_init"] for k, v in data["paramDevice"].items() if "P_init" in v
    }
    data["paramDeviceEnergyInitially"] = {
        k: v["E_init"] for k, v in data["paramDevice"].items() if "E_init" in v
    }
